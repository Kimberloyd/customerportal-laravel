...
const Dialog = ({
  ...
  return (
    <PrimitiveDialog
      ...
      role={role}
      className={cn(
        'peer/dialog group/dialog relative flex max-h-[calc(var(--visual-viewport-height)-var(--visual-viewport-vertical-padding))] flex-col overflow-hidden outline-hidden [--gutter:1.5rem] sm:[--gutter:2rem]',
        className
      )}
      {...props}
    />
...
const DialogHeader = ({ className, ...props }: DialogHeaderProps) => {
  return (
    <div
      ...
      className={cn(
        'relative space-y-1 p-(--gutter) pb-[calc(var(--gutter)-0.75rem)]',
        className
      )}
    >
...
const DialogFooter = ({ className, ...props }: DialogFooterProps) => {
  return (
    <div
      ...
      className={cn(
        'isolate mt-auto flex flex-col-reverse justify-end gap-3 p-(--gutter) pt-[calc(var(--gutter)-0.5rem)] group-not-has-data-[slot=dialog-body]/dialog:pt-0 group-not-has-data-[slot=dialog-body]/popover:pt-0 sm:flex-row',
        className
      )}
      {...props}
    />
