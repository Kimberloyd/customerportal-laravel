...
import { createContext, type ReactElement, useCallback, useContext, useId, useMemo, useState } from 'react'
...
const cursorStyleDefault = {
  ...
  fillOpacity: 0.35,
  radius: 6,
  // Recharts re-renders this same cursor rect in place on every hovered
  // bar rather than mounting a new one, so transitioning its own x/width
  // (its SVG position/size attributes) makes it visibly glide from bar to
  // bar instead of jumping -- opacity alone (the earlier attempt here)
  // only smoothed the fade-in/out, not the movement between bars.
  className: 'transition-[x,width,opacity] duration-200 ease-out',
} as const
