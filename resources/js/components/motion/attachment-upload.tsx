...
function AttachmentRow({
  ...
  return (
    <motion.li
      ...
      className={cn(
        "flex min-h-14 items-center gap-1 rounded-md border border-border bg-muted/70 p-0",
        className,
      )}
    >
      <div className="relative isolate flex min-w-0 flex-1 items-center gap-3 self-stretch overflow-hidden rounded-md bg-background px-2 py-1">
        {failed ? (
          <span
            aria-hidden="true"
  ...
  return (
    <LayoutGroup id={inputId}>
      ...
      <motion.button
        ...
        className={cn(
          "group relative isolate flex min-h-52 w-full flex-col items-center justify-center overflow-hidden rounded-md bg-muted/65 p-0 text-center outline-none",
      ...
      >
        <span
          aria-hidden="true"
          className="absolute inset-0 -z-10 rounded-md border border-border bg-background transition-[border-color,background-color] duration-200 group-hover:border-muted-foreground/45 group-data-[dragging=true]:border-foreground/65 group-data-[dragging=true]:bg-muted/20"
        />
        <motion.span
          aria-hidden="true"
          animate={
          ...
          transition={ITEM_TRANSITION}
          className="mb-3 grid size-11 place-items-center rounded-full bg-muted text-foreground transition-colors duration-200 group-hover:bg-muted/80 group-data-[dragging=true]:bg-foreground group-data-[dragging=true]:text-background"
        >
      ...
      {items.length > 0 ? (
        <section className="mt-4" aria-labelledby={`${inputId}-attachments`}>
          <h3 id={`${inputId}-attachments`} className="sr-only">
            {attachmentsLabel}
          </h3>
