...
const Stepper = React.forwardRef(function Stepper({
  ...
  renderStepIndicator,
  stepLabels = [],
  // When the caller renders its own nav buttons elsewhere (e.g. in a modal's
  // footer) instead of using the built-in footer-container.
...
function StepIndicator({ step, label, currentStep, onClickStep, disableStepIndicators }) {
  ...
  return (
    <div className="step-indicator-column">
      ...
      {label ? (
        <span className={`step-indicator-label ${status === 'inactive' ? 'inactive' : ''}`}>{label}</span>
      ) : null}
    </div>
  );
}
