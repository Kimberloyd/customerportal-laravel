...
$allowedOrigins = array_values(array_unique(array_filter(array_map(
    static function (string $origin): ?string {
        ...
        // Reverb compares this value with PHP_URL_HOST from the request's
        // Origin header. Accept full origins in env files, but pass only the
        // exact hostname to Reverb (never broaden the list with a wildcard).
        return parse_url($origin, PHP_URL_HOST)
            ?: parse_url("//{$origin}", PHP_URL_HOST)
            ?: $origin;
    },
    explode(',', (string) env('REVERB_ALLOWED_ORIGINS', 'http://localhost:8090,http://127.0.0.1:8090')),
))));
