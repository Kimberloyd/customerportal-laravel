...
export interface HistogramChartProps {
  ...
  data?: number[];
  label?: string;
  /** Plural noun for one measured thing, in the reader's own words. */
  sampleLabel?: string;
  ...
  return (
    ...
    >
        ...
        <div>
          ...
          <p className="mt-0.5 font-sans text-sm tabular-nums text-neutral-500 dark:text-neutral-400">
            <Stat ready={status === 'ready'}>
              Based on {formatCount(total)} {sampleLabel}
            </Stat>
          </p>
        </div>
      ...
      <ChartState
        ...
        variant="bars"
        empty={{
          title: `No ${sampleLabel} yet`,
          description: `Once the first ${sampleLabel} are recorded, you'll see how they compare here.`,
        }}
        onRetry={onRetry}
      >
      <div className="relative w-full" style={{ height }}>
        {!ready ? null : (
          <svg
            ...
            role="img"
            aria-label={`${label}, based on ${formatCount(total)} ${sampleLabel}. ${pins
          ...
          >
            ...
            <g className="font-sans tabular-nums">
              {countTicks.map((tick) =>
                tick === 0 ? null : (
                  <text
                    key={tick}
      ...
      <ChartDataTable
        caption={`${label} — how many ${sampleLabel} fall in each range`}
