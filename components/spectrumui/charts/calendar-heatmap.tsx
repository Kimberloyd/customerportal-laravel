...
export interface CalendarHeatmapProps {
  ...
  cell?: number;
  label?: string;
  /** How the covered window reads in a sentence, e.g. "this year". */
  periodLabel?: string;
  /** Timestamp of the last real day. The current streak counts back from here,
   *  so a grid padded into the future does not read as a broken streak. */
...
export function CalendarHeatmap({
  ...
  const streaks = React.useMemo(() => {
    ...
    if (currentThrough != null) {
      end = -1;
      for (let i = data.length - 1; i >= 0; i -= 1) {
        if (data[i].t <= currentThrough) {
          end = i;
          break;
        }
      }
    ...
    let current = 0;
    for (let i = end; i >= 0 && data[i].value > 0; i -= 1) current += 1;
  ...
  return (
    ...
    >
        ...
        <div>
          ...
          <p className="mt-0.5 font-sans text-sm tabular-nums text-neutral-500 dark:text-neutral-400">
            <Stat ready={status === 'ready'}>
              {streaks.current === 0
                ? 'No streak right now'
                : `${streaks.current} ${streaks.current === 1 ? 'day' : 'days'} in a row`}
              {' · '}
              {streaks.longest === 0
                ? 'no longest streak yet'
                : `longest ${streaks.longest} ${streaks.longest === 1 ? 'day' : 'days'}`}
            </Stat>
          </p>
        </div>
        <div className="flex items-center gap-1.5 font-sans text-xs text-neutral-400 dark:text-neutral-500">
          Less
          {[0, 1, 2, 3, 4].map((step) => (
            <span
              key={step}
      ...
      <ChartState
        ...
        variant="grid"
        empty={{
          title: `No ${label} ${periodLabel}`,
          description: `Each square is one day. Days fill in as ${label} are recorded.`,
        }}
        onRetry={onRetry}
      >
      <div className="relative w-full overflow-x-auto">
        {!ready ? null : (
          <svg
            ...
            role="img"
            aria-label={`${formatCount(total)} ${label} ${periodLabel}. Longest streak ${streaks.longest} days.`}
            onPointerLeave={() => setHovered(null)}
          >
            <g className="font-sans">
              {monthMarks.map((mark) => (
                <text key={`${mark.x}-${mark.label}`} x={mark.x} y={11} fontSize={11} fill="currentColor">
                  {mark.label}
                </text>
              ))}
              {[1, 3, 5].map((row) => (
