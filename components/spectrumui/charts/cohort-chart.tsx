...
// Enough for the plural nouns this chart takes ("customers", "accounts").
// A caller with an irregular plural should pass the singular it wants.
...
export interface CohortChartProps {
  ...
  data?: Cohort[];
  period?: string;
  /** Plural noun for who is being grouped, in the reader's own words. */
  memberLabel?: string;
  label?: string;
  hue?: string;
  status?: ChartStatus;
  ...
  return (
    ...
    >
      ...
      <div className="mb-6 flex flex-wrap items-end justify-between gap-x-4 gap-y-2">
        <div>
          <p className="font-sans text-lg font-semibold text-neutral-950 dark:text-white">
            {label}
          </p>
          <p className="mt-0.5 font-sans text-sm tabular-nums text-neutral-500 dark:text-neutral-400">
            <Stat ready={status === 'ready'}>
              {data.length} {data.length === 1 ? 'group' : 'groups'} ·{' '}
              {formatCount(totalUsers)} {totalUsers === 1 ? singularize(memberLabel) : memberLabel}
            </Stat>
          </p>
        </div>
        <p className="font-sans text-xs tabular-nums text-neutral-500 dark:text-neutral-400">
          <Stat ready={status === 'ready'}>
            {period} 1 avg{' '}
      ...
      <ChartState
        ...
        empty={{
          ...
          description: `Groups appear here once ${memberLabel} come back in a later ${period.toLowerCase()}.`,
        }}
        onRetry={onRetry}
      >
      <div className="relative w-full overflow-x-auto">
        {!ready ? null : (
          <svg
            ...
            role="img"
            aria-label={`${label}. ${data.length} groups over ${periods} ${period.toLowerCase()}s. ${period} 1 average ${averages[1] == null ? 'not available yet' : formatPct(averages[1])}.`}
            onPointerLeave={() => setHovered(null)}
          >
            <g className="font-sans uppercase tracking-wider tabular-nums" fontSize={10} fill="currentColor">
              <text x={0} y={HEAD_H - 10}>Started</text>
              <text x={LABEL_W} y={HEAD_H - 10}>{memberLabel}</text>
              ...
              return (
                <g key={cohort.label}>
                  ...
                  <text
                    ...
                    dominantBaseline="middle"
                    fontSize={11}
                    className="fill-neutral-400 font-sans tabular-nums dark:fill-neutral-500"
                  >
                    ...
                    return (
                      ...
                      >
                        ...
                        <text
                          ...
                          fontSize={10}
                          fontWeight={500}
                          className={cn('pointer-events-none font-sans tabular-nums', onFillClass(intensity))}
                          style={{
            ...
            <g>
              ...
              <text
                ...
                fontSize={11}
                fontWeight={600}
                className="fill-neutral-950 font-sans dark:fill-white"
              >
                Average
              </text>
              {averages.map((value, col) =>
                value == null ? null : (
                  <text
                    ...
                    fontSize={10}
                    fontWeight={600}
                    className="fill-neutral-700 font-sans tabular-nums dark:fill-neutral-200"
      ...
      <ChartDataTable
        caption={`${label} — how many ${memberLabel} came back, by starting ${period.toLowerCase()}`}
        columns={[
          'Started',
          memberLabel.charAt(0).toUpperCase() + memberLabel.slice(1),
          ...Array.from({ length: periods }, (_, i) => `${period} ${i}`),
        ]}
        rows={data.map((c) => [
          c.label,
          c.size,
      ...
      />
      <p
        className="mt-2 h-4 font-sans text-xs tabular-nums text-neutral-600 transition-opacity duration-150 dark:text-neutral-300"
